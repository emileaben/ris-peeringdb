...
for ixlan in j['data']:
    for net in ixlan['net_set']:
        a = net['asn']
        ris_reach_asn_set.add( a )


### now go through all ASNs in PDB for population dynamics
counters={
    'ris': {}, #ris asns
    'reach': {}, #asns in nets reachable by ris via ixps
    'pdb': {} # all of pdb
}
pdb_asn_set = set()

all_url=f'https://www.peeringdb.com/api/net'
r=requests.get( all_url )
j = r.json()
for net in j['data']:
    asn = net['asn']
    pdb_asn_set.add( asn ) # check for duplicates?
    for f in features:
        fv = net[f] # feature value
        if asn in ris_asn_set:
            counters['ris'].setdefault(f, Counter())
            counters['ris'][f][fv] += 1
        if asn in ris_reach_asn_set:
            counters['reach'].setdefault(f, Counter())
            counters['reach'][f][fv] += 1
        counters['pdb'].setdefault(f, Counter())
        counters['pdb'][f][fv] += 1

# calculate various population sizes
pops = {}
pops['ris'] = len( ris_asn_set )
pops['reach'] = len( ris_reach_asn_set )
pops['pdb'] = len( pdb_asn_set )

## todo add a thingie for all the asns that are in ris reach but don't have pdb entry

for f in features:
    for pop in ['ris','reach','pdb']:
        if f in counters[ pop ]:
            print(f"### feature:{f} population:{pop}")
            for k,v in counters[ pop ][ f ].most_common():
                rel_v = 100.0 * v / pops[ pop ]
                print(f"{k}\t{v}\t{rel_v:.2f}%")
            print()
        else:
            print(f"### feature:{f} population:{pop} (NOT AVAIL)")
            print()
